...
import static io.varve.swath.sort.finalize.SortTestSupport.drain;
...
/**
 * {@link CascadeReducer} over in-memory sorted segments: ordering, whole-page disjoint forwarding,
 * overlapping-page row merge, duplicate-hook coverage, cascade beyond a tiny fan-in, and the
 * fd-bound note.
 */
class CascadeReducerTest {
    ...
    void mergesDisjointSegmentsInOrder() throws IOException {
        ...
        CascadeReducer<Integer> merge = new CascadeReducer<>(cmp, 512, io, DuplicateHook.NO_OP, SortMetrics.NO_OP);
    ...
    void disjointSegmentsExerciseThePageWholePath() throws IOException {
        ...
        CascadeReducer<Integer> merge = new CascadeReducer<>(cmp, 512, io, DuplicateHook.NO_OP, metrics);
    ...
    void duplicateHookFiresOnAdjacentEqualEntries() throws IOException {   // I3
        ...
        CascadeReducer<Integer> merge = new CascadeReducer<>(cmp, 512, io, hook, SortMetrics.NO_OP);
        List<ListEntry> out = drain(merge.merge(new ArrayList<>(List.of(a, b))));
    ...
    void disjointCountsAccumulateAcrossACascadePassAndTheFinalPass() throws IOException {
        // fanIn=2 with 3 segments forces exactly one cascade pass (CascadeReducer.onePass), so this
        // exercises the onePass lambda's accumulation (CascadeReducer.java's passCopyable/passInterleaved +
        ...
        SortTestSupport.CountingMetrics metrics = new SortTestSupport.CountingMetrics();
        CascadeReducer<Integer> merge = new CascadeReducer<>(cmp, 2, io, DuplicateHook.NO_OP, metrics);
    ...
    void cascadeDeletesOnlyItsOwnIntermediatesKeepingScratchBounded() throws IOException {
        ...
        // publish, may reclaim them (crash-recoverability fix; see CascadeReducer's class javadoc and the
        ...
        CascadeReducer<Integer> merge = new CascadeReducer<>(cmp, 2, io, DuplicateHook.NO_OP, SortMetrics.NO_OP);
        try (SortedEntryCursor cursor = merge.merge(segments)) {
    ...
    void effectiveFanInCapsPeakOpenStreamsPerPassUnderATinyMergeBudget() throws IOException {
        ...
        // heap, so the pass width CascadeReducer is CONSTRUCTED with is what bounds merge-phase memory —
        ...
        SortConfig config = SortConfigs.base()
                .withFanIn(512)
                .withMergePerStreamBytes(64L << 10)
                .withMergeBudgetBytes(3L * (64L << 10));
        ...
        CascadeReducer<Integer> merge = new CascadeReducer<>(cmp, config.effectiveFanIn(), io, DuplicateHook.NO_OP,
                SortMetrics.NO_OP);
        List<ListEntry> out = drain(merge.merge(segments));
    ...
    void progressCallbackAdvancesDuringIntermediateCascadePassesNotJustTheFinalPass() throws IOException {
        ...
        // CascadeReducer#merge), so every intermediate-pass batch is already in hand at this point —
        // strictly BEFORE the final (unwrapped, caller-owned) streaming pass below is ever drained.
        SortedEntryCursor cursor = merge.merge(segments, batches::add);
    ...
    void aCrashMidCascadeCanStillBeRedoneBecauseOriginalsSurvive() throws IOException {
        ...
        CascadeReducer<Integer> firstAttempt = new CascadeReducer<>(cmp, 2, io, DuplicateHook.NO_OP, SortMetrics.NO_OP);
        ...
        assertThat(segments).allMatch(io::isLive);
        CascadeReducer<Integer> redo = new CascadeReducer<>(cmp, 2, io, DuplicateHook.NO_OP, SortMetrics.NO_OP);
