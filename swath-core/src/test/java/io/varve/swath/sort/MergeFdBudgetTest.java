...
/**
 * The pure, injectable fan-in clamp arithmetic ({@link MergeFdBudget}). Exercised with
 * injected limits so the fd clamp is proven without depending on the real process {@code ulimit}.
 */
class MergeFdBudgetTest {

    @Test
    void fdBoundedFanInReservesHeadroomBelowTheSoftLimit() {
        assertThat(MergeFdBudget.fdBoundedFanIn(1024, 128)).isEqualTo(896);
        assertThat(MergeFdBudget.fdBoundedFanIn(200, 128)).isEqualTo(72);
    }

    @Test
    void fdBoundedFanInFloorsAtTwoUnderAnExtremelyLowLimit() {
        assertThat(MergeFdBudget.fdBoundedFanIn(129, 128)).isEqualTo(2);   // 1 would be below the floor
        assertThat(MergeFdBudget.fdBoundedFanIn(10, 128)).isEqualTo(2);    // negative before the floor
    }

    @Test
    void fdBoundedFanInTreatsANegativeSoftLimitAsUnlimited() {
        // RLIM_INFINITY sentinel: no fd constraint at all.
        assertThat(MergeFdBudget.fdBoundedFanIn(-1, 128)).isEqualTo(Integer.MAX_VALUE);
    }
    ...
    void clampedFanInReducesFanInWhenTheSoftLimitIsLow() {
        ...
        int clamped = MergeFdBudget.clampedFanIn(512, 200, 128, Integer.MAX_VALUE);
        assertThat(clamped).isEqualTo(72);
    }

    @Test
    void clampedFanInIsUnaffectedByAHealthyFdLimit() {
        int clamped = MergeFdBudget.clampedFanIn(512, 10_000, 128, Integer.MAX_VALUE);
    ...
    void clampedFanInNeverReturnsBelowTwo() {
        assertThat(MergeFdBudget.clampedFanIn(512, 129, 128, Integer.MAX_VALUE)).isEqualTo(2);
        assertThat(MergeFdBudget.clampedFanIn(512, 10_000, 128, 1)).isEqualTo(2);
    }
}
