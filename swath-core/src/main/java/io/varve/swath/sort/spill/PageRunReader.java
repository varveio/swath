...
/**
 * Shared read-side IO/framing/validation for one {@link PageRunWriter} page-run segment.
 ...
 */
public final class PageRunReader implements AutoCloseable {
    ...
    private PageRunReader(FileChannel channel, Path path, SortMetrics metrics, int magic,
     ...
     */
    public static PageRunReader open(Path path, SortMetrics metrics) throws IOException {
    ...
    public static PageRunReader openUsingPersistedMaximum(Path path, SortMetrics metrics)
            throws IOException {
        return open(path, metrics, -1);
    }

    /** Open with the decoded-page maximum admitted for this segment by kickoff planning. */
    public static PageRunReader open(Path path, SortMetrics metrics, int maxRawPayloadLength)
            throws IOException {
        ...
        try {
            ...
            if (size < minSize) {
                ...
                throw new PageRunCorruptionException(path,
                        PageRunCorruptionException.PAGE_RUN_HEADER_CORRUPTION,
            ...
            byte[] tailBytes = readAt(channel, path,
                    size - PageRunWriter.TRAILER_FIXED_TAIL_BYTES,
                    PageRunWriter.TRAILER_FIXED_TAIL_BYTES).array();
            ...
            trailerCrc.update(tailBytes, 0, PageRunWriter.TRAILER_FIELDS_BYTES);
            if ((int) trailerCrc.getValue() != expectedTrailerCrc) {
                metrics.recordStealReason("SORT", "page_run_trailer_corruption");
                throw new PageRunCorruptionException(path,
                        PageRunCorruptionException.PAGE_RUN_TRAILER_CORRUPTION,
                        "fixed trailer CRC32C mismatch");
            }
            if (trailerMagic != PageRunWriter.MAGIC) {
                metrics.recordStealReason("SORT", "page_run_trailer_corruption");
                throw new PageRunCorruptionException(path,
                        PageRunCorruptionException.PAGE_RUN_TRAILER_CORRUPTION,
                        "bad or missing page-run trailer magic (truncated segment?)");
            }
            long fixedTailStart = size - PageRunWriter.TRAILER_FIXED_TAIL_BYTES;
            ...
            return new PageRunReader(channel, path, metrics, magic, version,
     ...
     */
    public record Page(byte[] body, PageBlockFormat.Header header) {
        public PageBlock decode(Path sourcePath) {
            return PageBlockFormat.deserialize(body, header, sourcePath);
        }
    }
    ...
    public record RoutingPage(long ordinal, long offset, int framedLen,
                       PageBlockFormat.RoutingHeader header) {
    }
    ...
    public Page nextPage() throws IOException {
        if (pagesRead == totalRecords) {
            if (nextFrameOffset != trailerStart) {
                throw corruption(PageRunCorruptionException.PAGE_RUN_BODY_CORRUPTION,
    ...
    public RoutingPage nextRoutingPage() throws IOException {
        ...
        try {
            ...
            header = PageBlockFormat.parseRoutingHeader(bodyLength,
    ...
    public PageBlock readPage(PageRef ref) throws IOException {
        ...
        return PageBlockFormat.deserialize(body, header, path);
    }
     ...
     * ({@link PageRunWriter#flush()} establishes the ascent by sorting pages on their first key).
     */
    private void checkMinMonotonic(byte[] newMin) throws IOException {
        ...
        throw new PageRunCorruptionException(path, PageRunCorruptionException.PAGE_RUN_MIN_REGRESSION,
                "page minKey regressed within segment (page " + pagesRead + " of " + totalRecords
                        ...
                        + " minKey order (PageRunWriter#flush sorts them); every merge path relies on"
    ...
    private void validateRoutingHeader(PageBlockFormat.RoutingHeader header) {
    ...
    private void requirePlannedDecodedPayload(int rawPayloadLength) throws IOException {
        if (rawPayloadLength > maxRawPayloadLength) {
            ...
            throw corruption(PageRunCorruptionException.PAGE_RUN_DECODED_PAGE_LIMIT,
    ...
    private void requirePersistedKeyMaximum(byte[] minKey, byte[] maxKey) throws IOException {
        ...
        if (observed > persistedMaxKeyLength) {
            ...
            throw corruption(PageRunCorruptionException.PAGE_RUN_KEY_LENGTH_LIMIT,
    ...
    static PageBlockFormat.Header parsePageHeader(byte[] body) {
        ...
        if (header.codec() == PageCompression.NONE
    ...
    PageRunCorruptionException corruption(String errorClass, String message, Throwable cause) {
        return new PageRunCorruptionException(path, errorClass, message, cause);
    }
