 ...
 * {@link SortedEntryCursor} over the surviving &le;{@code fanIn} segments.
 *
 ...
 * RUNTIME-clamped fan-in {@link io.varve.swath.output.sorted.SortedDatasetCoordinator} computes — the MIN of the static
 * {@link SortConfig#effectiveFanIn()} budget derived from {@link SortConfig#mergePerStreamBytes()},
 * the fd clamp ({@link FileDescriptorBudget}), and the per-segment encoded-record refinement. Active merge
 ...
 * reclaimed by the caller only after a successful publish ({@link io.varve.swath.output.sorted.SortedDatasetCoordinator} already tracks and
 ...
 */
final class CascadeReducer<S> {

    private static final Logger log = LoggerFactory.getLogger(CascadeReducer.class);
     ...
     * streaming pass. Shared here (rather than duplicated in {@link io.varve.swath.output.sorted.SortedDatasetCoordinator}) so both passes
    ...
    interface SegmentIo<S> {
        ...
        S writeIntermediate(SortedEntryCursor sorted) throws IOException;

        /**
         * Reclaim a segment. {@link CascadeReducer} only ever calls this on a segment it created itself via
    ...
    CascadeReducer(Comparator<ListEntry> comparator, int fanIn, SegmentIo<S> io, DuplicateHook hook,
              SortMetrics metrics) {
        if (fanIn < 2) {
     ...
     */
    SortedEntryCursor merge(List<S> segments) throws IOException {
     ...
     */
    SortedEntryCursor merge(List<S> segments, LongConsumer progressCallback) throws IOException {
     ...
     */
    private SortedEntryCursor openMerger(List<S> group, MergeRunSink disjointSink)
            throws IOException {
        ...
        try {
            ...
            return new DuplicateReportingCursor(new CascadePageMerger(
    ...
    private List<S> onePass(List<S> segments, boolean segmentsAreOriginals, LongConsumer progressCallback)
        ...
        for (int i = 0; i < segments.size(); i += fanIn) {
            ...
            try (SortedEntryCursor m = openMerger(group,
    ...
    /**
     * Wraps a {@link SortedEntryCursor} so rows drained through it advance {@code progressCallback}
     ...
     */
    private static final class ProgressTrackingCursor implements SortedEntryCursor {
        private final SortedEntryCursor delegate;
        private final LongConsumer progressCallback;
        private long batchRows;

        ProgressTrackingCursor(SortedEntryCursor delegate, LongConsumer progressCallback) {
