...
`local_publication_ms` expose those components. `manifest_md5_bytes`/`manifest_md5_ms` describe the
incremental byte-exact digest work. `manifest_bounds_rows`/`bytes`/`ms` describe only a post-close
