...
- `pipeline_header_scan_ms` sums metadata-read service across `PageRunHeaderStreams`. Cursor
...
| `emit` | `swath.emit.latency` | per page: the consumer stage's whole sink write (format+write for text, pool dispatch for Parquet, lane admission for `--sort`), including that stage's own row-tally merge (the per-entry tally itself is computed on the fetch worker that built the page). For `--format discard`, this is only the retained emission accounting; there is no formatter or write. |
| `channel_receive` | `swath.channel.receive.latency` | per envelope (every page plus the terminal `End`/`Failure`): the consumer stage's own wait to take the next envelope off the shared channel — the complement of `emit` on the consumer's timeline, so consumer wall ≈ Σ `emit` + Σ `channel_receive`. Long waits at a low page rate mean an idle consumer (the producers are slower than the sink); a per-page cost that stays flat while `writer_backpressure` is high means the consumer is paying for the channel itself (the sender wakeup convoy of varveio/swath#206 showed here as ~260 µs/page). |
...
Periodic data-sync evidence remains in `meters[]`, not `client_cost[]`: the format-tagged
`swath.data_sync.latency` timer records each data-only force on its writer thread,
...
cost on top of it; and (2) `emit`, `channel_receive` and `writer_backpressure`
...
included in either timer. `encoder_page_reads` counts positional full-frame reads, while
`encoder_read_wait` sums their service across encoders and can exceed wall time when reads overlap.
The decoded-page peak and open-part gauge complete the mechanism's residency and concurrency view.
...
| `FATAL` | `seed_endpoint_unreachable` | the explicitly seed-scoped fresh-run fetcher reached a connection-refused or unknown-host cause. The endpoint configuration cannot work for this invocation, so the seed fails on its first swath-owned attempt instead of spending the transient retry budget; ordinary and mid-run delimiter-probe resets remain retryable | |
...
| `SORT` | `staging_retained` | retention reconciliation preserved exactly the checkpoint-tracked original page-run staging under `sort.keep-staging=on`; every other immediate staging entry remains disposable. Fires once at the worker that owns the action: either actual merge completion or a PUBLISHED re-entry, never both in one dispatch. The accompanying `sort_staging_retained` log classifies `source=merge\|published_reentry` and carries retained/removed entry counts | |
...
| `SORT` | `buffer_page_raw_key_regression` | a comparator-disordered admitted page also contained a raw key lower than its predecessor, so the page-run writer rejected it before writing/finalizing the segment. This is a hard durable-cursor guard: repairing such a page could persist rows above the admission-time `lastKey` checkpoint maximum and make resume relist them. Expected 0 for valid S3 pages; nonzero aborts the run with `error_class=page_run_raw_key_regression` and no `StagedRunCommitter` checkpoint publication | |
...
| `SORT` | `pipeline_cluster_merge` | the router found one transitively overlapping page component. Fires once per cluster, not per page or row; the assigned encoder later reads its refs and admits pages incrementally to the bounded row merger. Read with `pipeline_cluster_pages` and `pipeline_cluster_rows` to classify overlap shape, and `pipeline_cluster_refs_peak` for the widest single component | |
| `SORT` | `pipeline_cluster_spilled` | one transitive overlap component outgrew the effective plan reference cap, so the router appended its remaining page references to a staging reference file instead of retaining them. Fires once per spilled component, not per reference. A component wider than the cap is legal input — one broad page can overlap every page of another run — and it is indivisible, so it takes an output part of its own. `pipeline_cluster_refs_peak` records the widest component observed, spilled or not — compare it with the effective plan reference cap; nonzero `pipeline_cluster_spilled` counts predict staging reference files under `cluster-*.pagerefs.tmp` for the life of the merge | |
...
| `SORT` | `pipeline_encoders_heap_clamped` | admission reduced the requested encoder count after pricing bounded scanner/header/frontier refs, the two router plan-ref waves (the current part and the overlap component being closed beside it) plus each encoder's queued and executing plan refs, one transient positional-read body, one conservative retained page, and an open writer per encoder. The retained-byte cluster budget is divided across admitted encoders. The companion warning carries the same resource fields as the FD clamp | |
| `SORT` | `pipeline_encoder_heap_floor_exhausted` | the fixed scanner/header/frontier references, both router plan-ref waves plus the queued/executing-plan references, one transient positional-read body, one dictionary-safe retained page, and one writer did not fit `merge-budget-bytes` even at one encoder and the effective plan floor (`min(initial refs, 256)`). Admission refuses resumably before shared channels or output writers open | |
| `SORT` | `pipeline_plan_ref_capped` | the planner lowered the runtime plan cap below 16,384 so the reference wave fit `merge-budget-bytes`, or the router reached the effective cap. Admission fires once with requested/effective limits in the warning; routing fires once per cap-driven close. The cap can create a part before the calibrated `final-file-bytes` target. An indivisible transitive overlap component that crosses it is not refused: it spills its references (`pipeline_cluster_spilled`) and takes a part of its own, because splitting the component could interleave rows across output parts | |
...
| `SORT` | `cross_part_overlap_rejected` | closed parallel final parts failed strict raw-byte adjacency before any stale-final sweep or rename. Fires once for the first offending pair; the fatal failure is `error_class=sort_output_order_regression` | |
| `SORT` | `cross_part_bounds_fallback_scan` | one closed parallel final-part writer did not expose close-gated raw bounds, so the pre-publication disjointness guard scanned that part's key column for exact first/last bytes. Fires once per fallback-scanned part; the scan checks merge cancellation cooperatively. Production sorted-Parquet writers normally provide bounds inline, so this is a library/compatibility path | |
| `SORT` | `completion_marker_empty_page` | a terminal listing page contained no in-range rows, so the sorted-output producer sent an explicit zero-row node-completion marker to retire the debug ordering tripwire state. Absent on non-sort runs and on non-terminal empty pages | |
| `SORT` | `completion_marker_filtered_page` | a terminal listing page had in-range rows but all were removed by configured filters, so the sorted-output producer sent an explicit zero-row node-completion marker to retire the debug ordering tripwire state. Absent on non-sort runs and when any row survives filtering | |
...
| `OUTPUT` | `data_sync` | an open output part completed at least one optional physical-byte data force. Fires once on first actual engagement, never merely because `--writeback-size` was configured; sync count/latency/bytes and final residual are separate bounded meters | |
...
| `SORT` | `merge_fanin_writer_reservation_degraded` | cascade width would have fallen below the two-stream minimum while descriptors were reserved for every requested output writer, so the reservation was lowered until the second input stream fit. The requested encoder count is a ceiling, not a commitment — `pipeline_encoders_*` still settles the lanes that actually run — so this is a merge that proceeds, never one that is refused. Fires once per affected merge, with the requested and reserved writer counts in the companion debug line | |
| `SORT` | `merge_fanin_floor_exhausted` | the fd and/or record-sized capacity could not open even the minimum two-stream cascade width, at the minimum one-writer output reservation, and two or more source segments require one — admission refuses resumably (`CascadeCapacityExhaustedException`) before any reader opens. A wider requested output alone never triggers this: the reservation is degraded first (`merge_fanin_writer_reservation_degraded`). A single source segment never triggers it either: it never opens a cascade group regardless of capacity | |
...
width. Color is **seed lineage**, not worker id — a reader cares which original guess the work
...
`ReplayMetrics` (`io.varve.swath.replay.metrics`) — HTTP/fixture/DuckDB-query tiers:

| Meter | Type | Meaning |
| --- | --- | --- |
| `swath.replay.http.requests` / `.http.errors` | counter | Requests served / 5xx responses. |
| `swath.replay.http.request.latency` | timer | Handler work through rendering and any injected delay, recorded before the socket write as in the original S3 server (p50/p99 published). |
| `swath.replay.request.stage.latency{protocol,stage}` | timer | Shared request stage cost for `page` (including outer read-permit wait), `render`, an engaged injected `delay`, and `write` through Jetty callback completion. |
| `swath.replay.response.admission.refused{protocol,reason}` | counter | Intentional count/byte budget refusals and shutdown admission refusals. The reason separates `response_count_exhausted`, `response_budget_exhausted`, `response_too_large`, and `shutdown`. |
| `swath.replay.response.write.deadline{protocol,reason=total_deadline}` | counter | Total response-write deadline expired and closed the client connection before callback completion. Distinguishes write timeout from an idle disconnect. |
| `swath.replay.response.bytes.live` / `.bytes.peak` / `.response.active` | gauge | During paging the initial output reservation is charged. During encoding the charge is `max(initial reservation, allocated chunk capacity)`; after encoding unused credit returns, leaving actual chunk capacity until callback cleanup. Response count permits are held through callback cleanup. Decoded page rows and native buffers are separate resource terms. |
| `swath.replay.response.chunk.allocation{reason}` / `.chunk.capacity.bytes{reason}` | counter / distribution | Successful charged chunk allocations and their capacities. Reasons distinguish initial, continuation, and cap-limited final chunks; the initial reason is split again when the response cap limits its capacity. |
| `swath.replay.response.percent.encoding.path{protocol,reason}` | counter | URL values encoded on successfully rendered pages through an already-charged chunk (`charged_chunk_one_pass`) or the exact-length fallback (`exact_length_fallback`). Primitive per-output counts are published once per page, not through per-key registry calls; failed renders do not contribute. This is currently the S3 percent-encoding grammar, not Azure's distinct encoded-name grammar. |
| `swath.replay.protocol.requests.active{protocol}` | gauge | Provider requests still paging, rendering, delayed, or waiting for their response callback. |
| `swath.replay.protocol.http.requests{protocol,status_class}` | counter | New per-protocol response count grouped into bounded HTTP status classes; aggregate `http.requests` retains its original identity and boundary. |
| `swath.replay.protocol.objects` / `.prefixes` / `.encoded.bytes{protocol,shape}` | counter | Selected object and grouped-prefix entries plus encoded body bytes for successfully rendered native pages. `shape` is `page`, `seek`, `delimiter`, or `unknown`; error envelopes are counted only by the status-class meter. |
| `swath.replay.provider.path{protocol,path,reason}` | counter | Provider pager path engagement and the classification that selected it, including indexed and fallback delimiter routes. |
...
| `swath.replay.page.read.latency` | timer | Post-borrow bounded-page decode service time (p50/p99 published), with reader-pool wait excluded. DuckDB records one sample per materialised query after connection acquisition. Sorted ordinary serving records one per range reader/file actually touched, timed inside `SortedParquetRangeReader` after its borrow; cache hits, empty/out-of-range reads that acquire no reader, and routing-index-only delimiter hops record none. A native delimiter skip-scan's key cursor is covered by the logical `parquet.query.latency`; when it materializes bare objects, their bounded full-row decode on the already-owned row-group reader adds one `page.read` sample. This is backing decode cost, not amortized HTTP-page cost: use `prefetch.window.fill` for a miss's end-to-end fill and `request.latency{shape}` for client-visible server cost. |
...
| `swath.replay.parquet.queries.in_flight` / `.queries.peak` | gauge | Current and run-peak **acquired backing readers**, not logical requests (pool wait excluded). DuckDB has one pool and is bounded by `connections`. Sorted mode has independent `connections`-wide range-reader and row-group-reader pools per file: an ordinary range request owns one range reader; a native delimiter request owns one row-group reader and reuses it when materializing bare objects. Concurrent requests of both shapes can engage both pools, but one request no longer occupies two readers; the conservative aggregate bound remains `2 × connections × files`. Cache lookup/hits and routing-index-only delimiter hops acquire none. |
| `swath.replay.delimiter.skipscan.row_group_opens` / `.whole_group_shortcuts` | counter / counter | Cursor/page-index opens that required Parquet access, and row groups resolved from adjacent routing bounds with no Parquet access. A common-prefix reseek can reopen the same physical row group, so `row_group_opens` counts operations, not unique groups. |
| `swath.replay.delimiter.skipscan.decoded_key_rows` / `.page_reseeks` | distribution / distribution | Per native delimiter rollup, key rows decoded by its cursors and same-physical-row-group common-prefix successor hops reseated through the page index. These distinguish page-bounded landing work from a sequential subtree walk. Both record one sample per engaged rollup, including zero. |
| `swath.replay.delimiter.reader_pool.open.latency` / `.readers_opened` | timer / distribution | One sample when a file's lazy delimiter reader pool is first touched: synchronous full-pool open time and number of reader handles opened. These isolate cold pool construction from subsequent skip-scan query time. |
| `swath.replay.prefetch.window.hit` / `.window.miss` | counter | Sequential-window prefetch engagement: a `rows()` call served from one or two compatible buffered windows (`hit`) vs. a delegate fill (`miss`). A high miss rate can mean bounds/projections are incompatible, windows are exhausted, or the useful working set exceeds the cache. Miss `reason` is `cold` or `continuation`. |
...
| `swath.replay.prefetch.windows.live` / `.rows.live` / `.anchors.live` | gauge | Current window count, decoded cached rows, and continuation-anchor count. Entries stay under `max-windows`; retained rows stay under `max-windows × window-rows` even when an individual continuation fill rounds above `window-rows`. |
...
| `swath.replay.prefetch.window.eviction` / `.row_budget_eviction` | counter | LRU window evictions and the subset forced by the global decoded-row budget. |
| `swath.replay.prefetch.window.selection{reason}` | counter | One outcome per lookup: exact-bound single window, safely contained upper bound, two-window join, insufficient candidates, or no covering candidate. Binary search and candidate qualification run outside the map lock. |
| `swath.replay.prefetch.window.join{reason}` / `.uncached{reason}` | counter | A successful two-window join or one declined join outcome per miss with a partially covering window; uncached reasons include oversized requests and rounded fills that cannot fit the global row budget. |
| `swath.replay.prefetch.window.ramp_ceiling_rows` | distribution | Page-aligned maximum fill size observed when an exclusive continuation anchor was claimed; 13,013 for a 1,001-row request and 15,003 for 5,001 rows at the default 12,500-row window. |

`FixtureMetrics` (`io.varve.swath.replay.fixture`, a deliberate **sibling** to `ReplayMetrics` —
`.fixture` does not depend on serving code) — the `sort-fixture`
...
| `swath.replay.serving.refused{reason}` | counter | A request the sorted path had to refuse outright, tagged with the typed reason from `io.varve.swath.output.parquet.sorted.RowGroupOrderException`: `row_group_disorder` (rows decoded by the `delimiter=/` skip-scan cursor were not in strictly ascending key order). NOT a `serving.fallback` reason: eligibility already passed (it proves the ascent of row-group *first* keys only), the serving path was chosen at startup, and nothing can take the request over — it fails `500`. Page-index and whole-group shortcuts do not decode every fixture row, so this is a request-path fail-stop signal, not an exhaustive fixture validation result. Bumped BEFORE the throw so the observed exclusion survives into sweep metrics. |
