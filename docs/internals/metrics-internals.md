...
| `swath.replay.parquet.query.latency` / `.query.errors` | timer / counter | One fixture read / failures (p50/p99 published). Both stores emit it: role-1's materialised DuckDB query, and role-2's page-index range read or delimiter skip-scan (no query engine on that path since the sorted store moved to Parquet's own page index). |
