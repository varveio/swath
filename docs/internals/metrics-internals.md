...
| `SORT` | `equal_key_rejected` | the final drain rejected the second row of an adjacent equal raw-key pair under `EqualKeyPolicy.REJECT`. Fires exactly once for the failed `sort-fixture` run, before the offending row is written; absent for successful fixtures and live `--sort`, which uses `ALLOW` | |
...
| `SORT` | `merge_scoped_frontier_trailer_reread` | one range-scoped page frontier had no retained descriptor and reread the trailer while opening. Fires only on the direct/test-support `ParallelRangeMerge` construction route; production merge kickoff supplies validated descriptors | |
...
| `swath.replay.sort.merge.boundaries.embedded.entries` / `.embedded.bytes` / `.scan.bytes` | counter | Boundary-selection embedded sample entries/bytes and fallback page-scan bytes observed by the fixture-sort adapter. |
