...
| `SEED` | `open_tile_sentinel_declined` | the scope could NOT be closed, and which precondition refused is deliberately recorded rather than left silent (a slow run wants to know the tail was open on purpose): the top page was truncated (unseen siblings may sort past what was observed, so no bound is provable); or the greatest returned item was a direct object sorting PAST the last common prefix (that prefix's ceiling would NOT bound it — appending it would cut the keyspace short, so this is a comparison, never an assumption); or the cut set already fills `targetSeeds` (never evict a real boundary for a sentinel); or the bound was not strictly above the current last cut | |
