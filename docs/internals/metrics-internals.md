...
| `SORT` | `merge_range_sample_capped` | the parallel merge's boundary sampling THINNED its candidate keys on a page-run segment: the segment held more pages than `MAX_SAMPLES_PER_SEGMENT` (4096), so it retained one page min-key per stride rather than one per page (`ParallelRangeMerge#sampleKeys`). The cap exists because an unthinned walk retains a key per ~1000 rows — ~1M cloned keys plus TreeSet overhead on a billion-row listing, held serially before any range starts. Fires once per capped SEGMENT, so the run total is how many segments were large enough to thin. Thinning cannot cost correctness (every row is assigned by an exact per-row compare, so boundary choice affects BALANCE only) — which is precisely why it needs a counter: a run whose ranges balanced badly has to be able to tell a thinned sample from a full one. Absent on small segments, on columnar staging, and on the serial path | |
