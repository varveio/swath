...
| `SORT` | `pipeline_encoder_heap_floor_exhausted` | the reference pipeline's fixed header/frontier/plan references, one transient positional-read body, one retained page, and one writer did not fit `merge-budget-bytes` even at the one-encoder floor. Admission refuses resumably before shared channels or output writers open; this is pipeline-specific and does not describe the range/serial minimum merge width | |
