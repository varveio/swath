...
| `SORT` | `merge_overlap_cluster` | page-aware merge entered one decoded overlap cluster. `sort.merge_overlap_clusters`, `.pages_peak`, and `.rows_peak` report how often this happened and the bounded decoded state retained at its largest point; the source-run counters above remain independent of page overlap | |
...
| `SORT` | `equal_key_rejected` | the final drain rejected the second row of an adjacent equal raw-key pair under `EqualKeyPolicy.REJECT`. Fires once per failing final drain, before the offending row is written; absent for successful fixtures and live `--sort`, which uses `ALLOW` | |
...
| `SORT` | `merge_range_page_skipped` | page skip on the parallel range-merge path: a range STEPPED OVER >=1 page whose `[minKey, maxKey]` frontier could not reach its `[lo, hi)`, so the page's rows were never decoded (`RangeScopedPageFrontier`). This does not avoid READING a skipped prefix page -- `PageRunSegmentIo#nextPage` CRC-verifies each framed body as it advances, and the trailer carries no per-page offset index -- but it avoids the front-code row decode. Fires once per range that skipped >=1 page, counting BOTH pages read-then-stepped-over (the below-`lo` prefix) and pages never read at all because the scan stopped at `hi` -- the latter is the larger saving, and for range 0 (`lo` unbounded) the only one. The skip fraction (`pages_kept` vs `pages_skipped` vs `pages_unread`) is in the per-range `sort_merge_range` log line. Absent only when a range spans the whole keyspace and on the serial path | |
...
| `SORT` | `page_run_entry_overlap_keymerge` | the generic entry-typed page-run read seam resolved overlapping adjacent pages at key level. Production path-backed merges use the decode-free frontier instead, so this is normally 0 outside direct seam tests or embedded `KWayMerge` users | |
| `SORT` | `page_run_min_regression` | the page-run READ-TIME format guard (HARD INVARIANT VIOLATION): a page-run segment stored a page whose `minKey` went BACKWARDS (unsigned) relative to the previous page in the same file. `PageRunSegmentIo#nextPage()` is the single page-advance primitive for both the decode-free `PageFrontierReader` and generic entry-typed `PageRunSegmentReader`; it increments this counter and aborts with `SegmentCorruptionException` / `error_class=page_run_min_regression`. Non-decreasing page mins are the lower-bound precondition for correct page-aware merging; legal range overlap does not fire it. Expected 0 for shipped-writer output; nonzero means corrupt/hand-built staging bytes or a writer bug, and no output is published | |
...
| `SORT` | `merge_fanin_mem_clamped` | encoded-record sub-reason of `merge_fanin_clamped`: the largest trailer `maxRecordLen` exceeded the configured `merge-per-stream-bytes` estimate enough to admit fewer priced stream slots. Fires alongside `merge_fanin_clamped`. This is a conservative allocation refinement, not a claim that encoded record length equals the stream's complete decoded resident heap | |
