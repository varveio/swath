...
| `swath.replay.parquet.queries.in_flight` / `.queries.peak` | gauge | Current and run-peak **acquired backing readers**, not logical requests (pool wait excluded). DuckDB has one pool and is bounded by `connections`. Sorted mode has independently sized range-reader and key-only delimiter-reader pools per file: an ordinary range request owns one range reader; a native delimiter request owns one delimiter reader and, only for a bare-object batch, temporarily owns one range reader too. The conservative aggregate bound is `(range connections + delimiter connections) × files`. Cache lookup/hits and routing-index-only delimiter hops acquire none. |
...
| `swath.replay.delimiter.reader_pool.wait.latency` | timer | One sample per successful delimiter-reader acquisition (p50/p99 published), measured from before lazy initialization or monitor entry through lease acquisition. It therefore includes first-fleet construction and contention; use `reader_pool.open.latency` to identify the construction subset. |
| `swath.replay.delimiter.range_reader_pool.wait.latency` | timer | One sample per bare-object lookahead delegation (p50/p99 published): time waiting for the ordinary range-reader pool while the delimiter lease remains held. This is the queueing-risk meter for delegation; post-borrow service remains in `page.read.latency`. |
| `swath.replay.delimiter.key_index.latency{state=first_load|reuse}` | timer | Key-only `ColumnIndexStore` lookup time (p50/p99 published) on a delimiter reader. `first_load` is the first successful lookup for that reader/physical-row-group pair; `reuse` is a reader-local cached lookup. |
| `swath.replay.delimiter.object_lookahead.delegations` | counter | Bare-object lookahead refills successfully delegated to the ordinary range-reader pool. A failed range read is not counted; common-prefix-only rollups leave it at zero. |
