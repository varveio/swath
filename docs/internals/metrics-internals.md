...
| `SORT` | `pipeline_encoders_fd_clamped` | pipeline-specific admission reduced the requested encoder count because, after fixed process headroom and one descriptor per surviving segment, fewer than `N` output-writer descriptors remained. The companion warning carries requested/effective counts, segment count, slot depth, and page price. It is independent of every `merge_range_*` reason | |
| `SORT` | `pipeline_encoders_heap_clamped` | pipeline-specific admission reduced the requested encoder count after reserving `K * slotDepth * page` reader-slot bytes and pricing each encoder as its four-page queue plus the open-writer heap estimate. No proof-spool or staged-size term participates. The companion warning carries requested/effective counts, segment count, slot depth, and page price | |
