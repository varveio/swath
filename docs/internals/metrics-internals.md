...
| `delimiter_fanout.{max,total,probes}` | `commonPrefixes().size()` at each seed + thief `delimiter=/` structure probe | the widest / total delimiter-child count observed and how many probes saw it — the bucket's branching factor. **Thief contributions saturate at `Thief.STRUCTURE_PROBE_MAX_KEYS`**, so on a bucket whose structure is discovered at steal time rather than at seed time this under-reads the true branching factor; `STRUCTURE.fanout_capped` says how often that happened. Seed probes are unaffected (they use the full `PROBE_PAGE`). |
...
| `IDLE_SLOT` | `in_flight` | an idle worker was refused a steal attempt because **another worker owns the sole in-flight slot** — the fleet-wide one-attempt bound holding. The worker goes on to park: on the seconds-scale in-flight backstop if the slot is still held when it re-reads the state under the ledger gate, or on the ordinary pacing base if the owner released in between. A high count against few `STEAL.attempted` is the expected shape of a slow probe, not a fault. Splits the aggregate `swath.idle_backoff.slot_denied`, which cannot tell this from `paced` | |
