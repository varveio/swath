...
| `OUTPUT` | `tsv_escape_on` / `tsv_raw_output` | cheap classification companion to `tsv_byte_encoder`: whether control-byte escaping was enabled or `--raw-output` selected verbatim controls. Exactly one fires with each `tsv_byte_encoder` engagement | |
