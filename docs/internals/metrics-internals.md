...
| `SORT` | `post_publish_cleanup_pending` | the authority listener returned (so this run's final parts plus manifest/state/symlink/last-written `_SUCCESS` are committed), but a later WP8 hook, disposable-intermediate cleanup, or staging completion failed. Fires exactly once per affected transform and maps to a non-fatal `PublicationPendingException`; it never means the dataset rolled back. The accompanying `sort_post_publish_cleanup_pending` warning carries `publication_committed=true`, `cleanup_pending=true`, and `stage=after_publish_listener_hook\|disposable_intermediate_cleanup\|original_staging_completion\|after_staging_completion_hook`. Managed resume enters PUBLISHED cleanup with zero LIST requests | |
