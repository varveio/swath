...
| `SORT` | `merge_fanin_floor_exhausted` | the fd and/or record-sized capacity could not open even the minimum two-stream cascade width and two or more source segments require one — admission refuses resumably (`CascadeCapacityExhaustedException`) before any reader opens. A single source segment never triggers this: it never opens a cascade group regardless of capacity | |
