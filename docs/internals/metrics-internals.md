...
| `SORT` | `pipeline_encoders_fd_clamped` | pipeline-specific admission reduced the requested encoder count because, after fixed process headroom and one shared descriptor per surviving segment, fewer than `N` output-writer descriptors remained, or refused before channel open when no writer descriptor remained. A successful clamp's companion warning carries requested/effective counts, segment count, cursor depth, key-sized reference price, transient read-body price, retained-page price, and cluster share. It is independent of every `merge_range_*` reason | |
