...
- **Engagement counters** — `metrics.recordStealReason(category, reason)`: *did this path fire, and how
  ...
  **`--engine-toggle` ablation marks.** `swath` exposes a diagnostic-tier `--engine-toggle
  ...
  line (`engine_toggles_effective owner_split=... density_ewma=... ...`, all 12 toggles) whenever
  any toggle is non-default, and the JSON run-summary's `engine_flags` block (§3) echoes all 12
...
| `TOGGLE` | `rate_anchored_sensing_on` | engagement mark, same polarity as `readahead_on` (fires only on the opt-in branch): the run steered victim choice and the owner-split gates on the PORTED position sensor (`RateAnchoredEstimator` — proven mass, banded by cursor-anchored geometry at the promoted quarter floor) instead of the shipped `StealMath.estRemaining` window reading (`--engine-toggle rate_anchored_sensing=on`). Fired once at engine construction, so a real-bucket A/B reads which arm a run was on from the run itself; the `SENSING.*` rows below are that sensor's own per-reading classification and are silent on a default run | |
| `SENSING` | `geometry_capped` | the ported sensor's cursor-anchored geometric factor read ABOVE `RateAnchoredEstimator.GEOMETRY_BAND`, so the lift it applied to the range's proven mass was capped at the band. One of five mutually-exclusive readings recorded per classified estimate — at the winning victim of a steal attempt and at every owner-split gate consult — so their distribution is what the band actually did on a bucket. Fires only under `rate_anchored_sensing=on` | |
| `SENSING` | `geometry_lift` | ...the factor read above 1 and inside the band, so measured position lifted the range's proven mass by exactly that factor. On a deep-nested keyspace this is the reading the shipped window cannot take at all (its consumed span underflows to zero and the emitted keys drop out of the estimate) | |
| `SENSING` | `geometry_neutral` | ...the factor read exactly 1: either there is no consumed evidence to anchor (cursor at or before `lo`, or a consumed span that underflowed) or the frame is exactly balanced, so the estimate is the range's proven mass unmodified | |
| `SENSING` | `geometry_cut` | ...the factor read below 1 but at or above the run's floor, so geometry cut the proven mass by the amount the floor still allows — the measured shortfall the floor deliberately keeps | |
| `SENSING` | `geometry_floored` | ...the factor read BELOW the floor (`QUARTER_MIN_GEOMETRY`) and the floor refused the rest of the cut. This is the counter the promotion exists for: read against `OWNER_SPLIT.remaining_est_floor`, it is where the shipped reading would have scored a straggler's proven mass low enough to refuse its carve | |
| `SENSING` | `page_floor` | the classified estimate's magnitude was the no-evidence page floor rather than proven mass (the range has emitted fewer keys than one page), so the reading is an assumption about a barely-started range and not a measurement of it. Co-fires with whichever geometry row above applies | |
