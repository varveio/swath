...
| `swath.replay.response.bytes.live` / `.bytes.peak` / `.response.active` | gauge | Charged encoded array capacity now/peak and response-count permits held until callback cleanup. Decoded page rows and native buffers are separate resource terms. |
