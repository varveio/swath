...
| `PROBE` | `slow_phase_` | the phase that dominated the slow probe above, suffixed `connect_acquire` (connection-pool/TLS acquisition — client-side starvation), `ttfb` (the store itself was slow to first byte), or `unknown` (the SDK published no phase timings for the call). The classification the aggregate `swath.s3.call_class_latency` distributions cannot make: they show that probes were slow, not which half of the call was | |
