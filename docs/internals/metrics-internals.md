...
| `SORT` | `pipeline_encoders_fd_clamped` | pipeline-specific admission reduced the requested encoder count because, after fixed process headroom and one shared descriptor per surviving segment, fewer than `N` output-writer descriptors remained. The companion warning carries requested/effective counts, segment count, cursor depth, reference price, page price, and cluster share. It is independent of every `merge_range_*` reason | |
| `SORT` | `pipeline_encoders_heap_clamped` | pipeline-specific admission reduced the requested encoder count after pricing bounded header/frontier refs, worst-case retained plan refs, and one decoded page plus an open writer per encoder. Cluster budget is divided across admitted lanes; no proof-spool or staged-size term participates. The companion warning carries the same resource fields as the FD clamp | |
