...
overlap routing engaged, whole-page/cluster work, header-scan service, plan back-pressure, positional
encoder reads, and open writers. `swath.sort.pipeline.router_wait` is the router's total blocked time;
`plan_queue_wait` identifies its full-lane component. `encoder_read_wait` is summed service time across
lanes and may exceed wall time when reads overlap. The summary exposes these fields directly.
...
| `SORT` | `pipeline_cluster_merge` | the reference router found a transitively overlapping page cluster. Fires once per cluster, not per page or row; the assigned encoder later engages the bounded shared page-row merger. Read with `pipeline_cluster_pages` and `pipeline_cluster_rows` to classify overlap shape | |
| `SORT` | `pipeline_encoders_fd_clamped` | pipeline-specific admission reduced the requested encoder count because, after fixed process headroom and one shared descriptor per surviving segment, fewer than `N` output-writer descriptors remained. The companion warning carries requested/effective counts, segment count, cursor depth, reference price, transient read-body price, retained-page price, and cluster share. It is independent of every `merge_range_*` reason | |
| `SORT` | `pipeline_encoders_heap_clamped` | pipeline-specific admission reduced the requested encoder count after pricing bounded header/frontier refs, capped complete-plan refs, one transient positional-read body, one conservative retained page, and an open writer per encoder. The retained-byte cluster budget is divided across admitted lanes; no proof-spool or staged-size term participates. The companion warning carries the same resource fields as the FD clamp | |
| `SORT` | `pipeline_plan_ref_capped` | the router's hard 16,384 refs-per-plan bound engaged. Normally it closes the current raw-key-disjoint plan before admitting another item, so even an unbounded `final-file-bytes` target retains a constant number of refs and may yield several parts. If one indivisible transitive overlap component alone crosses the cap, the same reason fires before the excess ref is retained and the merge refuses resumably because splitting that component could interleave rows across output parts. Fires once per cap-driven close or refusal, not once per ref | |
...
| `SORT` | `page_run_trailer_corruption` | the fixed page-run trailer fields failed CRC32C or the trailing magic was absent. The segment aborts with typed `page_run_trailer_corruption` before trusting counts or offsets | |
