...
| `SORT` | `merge_decoded_page_budget_exhausted` | truthful encoded/decoded page maxima made the pipeline's one-encoder floor exceed `merge-budget-bytes`. Finalization is refused before output allocation with non-fatal merge-pending disposition; raising the budget allows a zero-LIST retry | |
