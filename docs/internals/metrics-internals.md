...
| `swath.replay.parquet.queries.in_flight` / `.queries.peak` | gauge | Current and run-peak acquired backing readers (pool wait excluded). Sorted mode owns `--parquet-connections` readers per fixture file, so the aggregate peak is bounded by that value times the files concurrently touched; cache lookup and hits acquire none. |
