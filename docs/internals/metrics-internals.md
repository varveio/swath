...
- **Engagement counters** — `metrics.recordStealReason(category, reason)`: *did this path fire, and how
  ...
  **`--engine-toggle` ablation marks.** `swath` exposes a diagnostic-tier `--engine-toggle
  ...
  The default `reach_floored` mode therefore marks itself; the pre-0.2.0 `current` rollback does not.
...
| `SENSING_STEAL` | `geometry_lift` | ...the factor read above 1 and inside the band on the winning candidate, so measured position lifted the range's proven mass by exactly that factor — the reading that promotes a deep-nested range the pre-0.2.0 window scores as a bare width | |
...
| `SORT` | `merge_range_page_skipped` | page skip (off-by-default, parallel range-merge path), the page-run twin of `merge_range_rowgroup_skipped`: a range STEPPED OVER >=1 page whose `[minKey, maxKey]` frontier could not reach its `[lo, hi)`, so the page's rows were never decoded (`RangeScopedPageFrontier`). Unlike the Parquet row-group skip this does not avoid READING the skipped pages -- `PageRunSegmentIo#nextPage` CRC-verifies each framed body as it advances, and the trailer carries no per-page offset index -- it avoids the front-code row decode, which is the expensive part. Fires once per range that skipped >=1 page, counting BOTH pages read-then-stepped-over (the below-`lo` prefix) and pages never read at all because the scan stopped at `hi` -- the latter is the larger saving, and for range 0 (`lo` unbounded) the only one, so a counter that ignored it would report the first range as having skipped nothing. The skip fraction (`pages_kept` vs `pages_skipped` vs `pages_unread`) is in the per-range `sort_merge_range` log line. Absent only when a range spans the whole keyspace (nothing to skip) and on the serial path | |
| `SORT` | `merge_range_unsplittable` | a parallel merge was REQUESTED (`swath.sort.merge-parallelism>1`) but the keyspace could not be split into more than one range -- fewer than two distinct sampled keys -- so the run fell back to the untouched serial merge (`ParallelRangeMerge#boundaries` returned null). Fires at most once per run. Distinguishes "asked for parallelism and did not get it" from "never asked", which are otherwise identical in the metrics: engaged runs show `merge_range_parallel` instead, and a run that never set the knob shows neither | |
| `SORT` | `merge_range_clamped` | the requested `swath.sort.merge-parallelism` was REDUCED because the merge budget could not carry that many concurrent ranges over this run's staged-segment count (`ParallelRangeMerge#effectiveRanges`): past `merge-budget-bytes / (segments × per-stream-bytes)` every range would cascade, which is slower than the serial merge it replaces. Fires at most once per run, alongside a `sort_merge_range_clamped` WARN carrying the requested and effective `R`. This is the counter that makes an over-ambitious `R` visible at all — `merge_range_parallel` fires once per range whether or not the ranges cascade, so a run pessimised by its own tuning is otherwise indistinguishable from a well-tuned one. When the clamp lands below 2, `merge_range_unsplittable` fires instead and the run takes the serial path | |
