...
| `SORT` | `merge_seek_plan_index_unusable` | one segment's type-2 range-seek plan retained header starts because its index was absent, non-type-2, invalid, skipped, or valid but empty. Fires once per affected segment and is paired with exactly one `merge_seek_plan_index_status_*` classification below | |
| `SORT` | `merge_seek_plan_index_status_empty` | the unusable seek-plan index was valid type 2 but contained zero entries | |
