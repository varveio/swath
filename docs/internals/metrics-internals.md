...
| `SORT` | `pipeline_cluster_spilled` | one transitive overlap component outgrew the effective plan reference cap, so the router appended its remaining page references to a staging reference file instead of retaining them. Fires once per spilled component, not per reference. A component wider than the cap is legal input — one broad page can overlap every page of another run — and it is indivisible, so it takes an output part of its own. `pipeline_cluster_refs_peak` shows how far past the cap the widest component reached; nonzero counts predict staging reference files under `cluster-*.pagerefs.tmp` for the life of the merge | |
