...
These are overlapping writer work, never page-serial costs. For the first TSV/JSONL adapter, the
cadence is driven by physical post-compression bytes. The generic meter family is reserved for
separately benchmarked format adapters; its presence does not imply every format currently syncs.
