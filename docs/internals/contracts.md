...
| `--writeback-size` | disabled (`0`) | optional writeback-shaping cadence for physical bytes already emitted to an OPEN unsorted TSV/JSONL/Parquet directory-dataset part; positive values below 4 MB and unsupported output paths are rejected. A periodic data sync never closes/rotates the part, forces its directory, exposes digest metadata, advances a checkpoint, or makes it publishable; **it does not shorten the crash-recovery window**. Final close retains the full file+parent durability boundary (I6). Physical post-compression bytes drive the cadence. Text adapters do not flush their codec; Parquet flushes only its 4 KiB transport buffer and therefore can engage only after parquet-mr naturally emits a completed row group — it never flushes a row group/page/column store. `--text-part-size` and `--parquet-part-size` retain their existing logical rotation meanings. Sorted staging/final output, single-file output, and merge concurrency/fan-in remain outside this policy. Disabling row/time rotation to obtain size-only Parquet parts can widen checkpoint RPO; writeback does not compensate for that. |
