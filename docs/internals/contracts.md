...
| **Directory-dataset TSV/JSONL** | Bounded parallel lanes write independent parts; successful completion publishes one atomic manifest and `_SUCCESS` last. A failed run has neither marker and may leave finalized parts from that attempt. | **No.** Text datasets require `--checkpoint none` in this release. |
