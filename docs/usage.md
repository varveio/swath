...
| `carve_brake` | **off** | **Not yet promoted — raced separately from the toggles above.** `mass_k2`/`mass_k4`/`mass_k8` add a realized-child-mass carve brake AFTER the confetti feedback gate: it refuses a further carve whenever the recent window-average EFFECTIVE mass of the last up-to-8 tagged owner-split children drops below `K × max-keys` (`K` = 2/4/8), with every 16th would-be-braked carve let through anyway as a periodic probe (so a keyspace whose shape changes mid-run re-earns carving). **Zero warmup** (E-20 amendment): the window average is a prefix average from the FIRST tagged completion onward, never gated on a sample-count floor — a codex cross-read of a failing dense/uniform rep found damaging carves reaching this exact gate with a thin-unsplit signal already present, refused only because an earlier 8-completion warmup still read the window as "no signal yet"; dropping that warmup (not moving the gate) closed the gap. **Split-aware effective mass**: a child that itself split further is never counted at its own (possibly tiny) tally — `effectiveMass = hasSplit ? max(keysEmitted, K×max-keys) : keysEmitted` — mirroring the confetti gate's own rule that a healthy child which shed its own further tail(s) onward is proof the carve worked, not evidence of a thinning tail. Distinct from `confetti_feedback`: that gate reads a RATE of already-degenerate children; this reads the recent MASS TREND, catching an over-carve before its children turn confetti-sized — the intended fix for the dense/uniform instability `denseUniformShapeNeverEngagesTheGateAndSubstantialDominates` (`ConfettiFeedbackWiringTest`) discloses on the 0.2.0 default pair, and for a documented family of clean corpus regressions sharing the same over-carving signature. Tagging (which completions feed the window) is decoupled from `confetti_feedback`: a child is tagged, and its completion recorded into the window, whenever `carve_brake != off`, whether or not `confetti_feedback` is also on. Refuses SPLITS only — never page work, never stealing, never any other gate's decision — so `carve_brake=off` is byte-identical to a build that predates this toggle (decision-trace goldens pin this). A run on a non-`off` arm marks itself `TOGGLE.carve_brake_<mode>_on` and counts `OWNER_SPLIT.carve_braked`/`OWNER_SPLIT.carve_brake_probe`; the `owner_split_decision` trace event additionally carries the window-average mass reading it decided on (`carve_brake_mass_avg`, finite from the first tagged completion onward, omitted entirely when this toggle is `off` — metrics-internals.md §7). Rollback: `carve_brake=off` (the default in this commit). Which `K`, if any, ships as the promoted default is decided by a separate race, not this commit. |
