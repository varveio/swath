...
| `sort.merge-boundary-policy` | `distinct, or rows` | `distinct` | experimental | free | fresh list and resume | Choose parallel-merge split points. `distinct` preserves the shipped evenly spaced selection over the bounded distinct page-minimum sample. `rows` uses validated type-2 cumulative entry mass to approximate row quantiles; any extensionless, type-1, invalid, or mixed input falls back to `distinct` for the whole merge. Boundaries remain strictly increasing raw keys, so equal keys remain in one range. The default does not change without a separate benchmark decision. |
...
| `swath.sort.merge-boundary-policy` | `distinct` | Experimental parallel-range boundary policy. Prefer `--tune sort.merge-boundary-policy=distinct|rows`; the typed CLI value wins over this property. `rows` is default-off and falls back to `distinct` unless every original input carries a validated type-2 page index. |
