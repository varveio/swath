...
| `sort.merge-parallelism` | `integer 1..16` | core-derived, capped at `8` | stable | free | fresh list and resume | Set the maximum number of contiguous key ranges, or pipeline encoders when `sort.finalization=pipeline`, in the final sorted merge. Runtime heap, fan-in, staged-size, and file-descriptor gates may lower it. Each engaged range produces at least one final file; pipeline encoder count does not determine part count. Benchmark merge wall, peak memory, and consumer-visible file count together. A pre-publication resume may choose a different value because partial finals are disposable and finalization reruns from durable page-run segments. |
| `sort.finalization` | `ranges, or pipeline` | `ranges` | experimental | free | fresh list and resume | Choose the final sorted merge mechanism. `ranges` is the default key-space partitioner. `pipeline` is experimental and default-off: bounded header cursors feed one page-reference router, which sends complete ordered part plans through a shared bounded queue to positional-read encoders. It does not choose or seek to range boundaries. Use the same value for repeated comparisons, and retain part geometry and pipeline wait metrics with the result. |
...
| `sort.ignore-disk-check` | `on or off` | `off` | diagnostic | free | fresh list and resume | Bypass the startup/listing sorted-output checks and the range finalizer's merge-start free-space admission. The experimental pipeline has no merge-start `MergeDiskPlan` preflight. Size the staging volume independently first. |
