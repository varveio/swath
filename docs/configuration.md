...
`sort.merge-parallelism`, `sort.keep-staging`, and
...
| `swath.sort.merge-per-stream-bytes` | `64 KiB` | Configured per-stream floor used by cascade planning. The v4 fixed tail supplies the actual maximum record-body length; final encoder admission also uses its decoded-payload and key maxima. |
...
| `swath.sort.final-file-bytes` | `1 GiB` | Soft roll target for final sorted parts. Rolls occur only before a complete router item: one staging page or one transitive overlap component. A staging page and an equal-key group are never split across parts. The heap-admitted 256–16,384-reference plan cap may roll earlier, and an overlap component wider than that cap takes a part of its own. |
| `swath.sort.final-row-group-bytes` | `8 MiB` | Final Parquet seek granularity. Also prices each admitted encoder's writer-heap reservation (row-group bytes plus a fixed working-buffer allowance, floored at a measured minimum writer cost), so raising it admits fewer parallel encoders under the same `merge-budget-bytes` while lowering it below the default gains none. |
