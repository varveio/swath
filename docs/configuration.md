...
| `swath.sort.final-row-group-bytes` | `8 MiB` | Final Parquet seek granularity. Also prices each admitted encoder's writer-heap reservation (row-group bytes plus a fixed working-buffer allowance), so raising it admits fewer parallel encoders under the same `merge-budget-bytes`. |
