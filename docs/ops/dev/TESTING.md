...
| **"Millions of objects" realism** | the **S3 Listing Replay Server** (`:swath-replay`) — serves synthetic `ListObjectsV2` XML from a fixture (the LIST-only high-fidelity oracle; swath is LIST-only, no inventory path) | fixture-backed, cheap |
