...
| `pipeline_cluster_refs_peak` | Page references in the widest single transitive overlap component. Above the admitted plan reference cap the component spilled its references to staging and took an output part of its own. |
...
| Phase and output | `swath.phase`, `swath.output.files`, `swath.output.bytes`, `swath.output.broken_pipe`, `swath.emit.latency`, `swath.channel.receive.latency` | What phase is active and how the selected sink behaves. |
