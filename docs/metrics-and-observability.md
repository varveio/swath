...
| `swath.open_frontier.keys_emitted` | counter | — | keys committed while their node's upper bound was `null` — the serial-tail attribution gauge for the one range the owner-split governor can never self-split off (`OWNER_SPLIT.open_frontier`, §5). Absolute, like `swath.entries.emitted` it is a subset of; divide by it for the share of the run's mass that drained from the open frontier — a large share paired with a slow tail means the tail sat here, un-carvable by the owner, rather than gate-blocked |
