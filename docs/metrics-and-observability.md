...
| `swath.fetch.latency.phase` | timer | `{call_class, phase}` | per-call-class latency-phase decomposition — what tells a probe-timeout storm's client-side pool starvation apart from server-side slowness. `call_class` is `worker_page`\|`pivot_probe`\|`structure_probe`, classified purely from the request shape (`delimiter=/` = structure probe, `delimiter`-less `max_keys<=1` = pivot probe, everything else = a worker's range page fetch); `phase` is `connect_acquire`\|`ttfb`\|`sdk_unmarshal`\|`total`\|`response_parse`. `response_parse` is the CLIENT-side conversion of an already-received response into swath's page model — the one parse cost swath itself owns and can time, and the per-call-class member of the client-service-cost decomposition (§3 `client_cost[]`). `sdk_unmarshal` is the AWS **SDK's own** response-handling window — first response byte through the SDK's protocol response handler returning — measured from the SDK's stamps rather than by swath: on the sync `ApacheHttpClient` path the response body is still a live socket stream when that handler runs, so it spans draining the remaining response bytes off the wire **as well as** the XML parse and POJO construction, and is therefore not pure client CPU. **It is DERIVED, as `TimeToLastByte - TimeToFirstByte`, not read from a dedicated metric:** `CoreMetric.UNMARSHALLING_DURATION` would be the exact boundary, but it is never published for an S3 `ListObjectsV2` **by construction** — `DefaultS3Client` holds an `AwsS3ProtocolFactory`, which overrides `createCombinedResponseHandler` to route through its own `createErrorCouldBeInBodyResponseHandler` (S3 can return an error document under a `200`), and that path never goes through `AwsXmlProtocolFactory.timeUnmarshalling`, the decorator that reports the metric. `TIME_TO_LAST_BYTE`, by contrast, is stamped in the SDK's `HandleResponseStage` *after* the response handler returns, which makes the difference the response-handling window (verified against a real attempt — see `S3SdkUnmarshalPhaseLocalStackIT`). Read `sdk_unmarshal` as a close UPPER bound on the SDK's client-side response cost, not as a true unmarshal span. It **excludes** the SDK's response-INTERCEPTOR chain that runs after the handler — for S3, the percent-decode of the `encoding-type=url` response swath itself requests, which rebuilds the response object — and it excludes `response_parse`. So `sdk_unmarshal` narrows the `total`-minus-`ttfb` residual — measured as its dominant term — without closing it: that interceptor chain, everything request-side (`ttfb`'s zero is the ATTEMPT start, so marshalling, endpoint resolution, credential fetch and signing all sit in front of it), any retry backoff and every superseded attempt, plus swath's own fetcher overhead, are what is left. `connect_acquire` (the connection-pool checkout wait), `ttfb` and `sdk_unmarshal` come from the AWS SDK's own per-attempt metrics and are **best-effort** — any may be absent on a given attempt, and is then skipped, never fabricated as a `0` sample; `sdk_unmarshal` additionally needs BOTH stamps from the SAME attempt (an attempt reporting a first byte but no last byte clears any earlier attempt's window rather than letting it stand), a negative difference is rejected outright rather than clamped, and an exact-zero window is admissible as a real sample. `total` is swath's own measured wall-clock elapsed. Bounded cardinality (3×5 = 15 series max). Publishes p50/p90/p99; read them back from the JSON summary's `probe_latency[]` (§3), since the generic `meters[]` readback carries a Timer's count/total_ms/max_ms only, not percentiles. **The phases are NOT additive:** the SDK does not say whether time-to-first-byte is measured from request-issue or from after the connection is already leased, so `connect_acquire` and `ttfb` may partially overlap — read each as its own independent signal, never as phase shares that sum to `total`. `sdk_unmarshal` does follow `ttfb` in wall-clock (the response handler cannot run before the first byte arrives), so subtracting the two from `total` is meaningful — but it is **not** a wire/CPU split: because the sync unmarshaller pulls the body straight off the socket, `sdk_unmarshal` contains the download of everything after the first byte as well as the parse. Read it as an UPPER bound on the SDK's client-side response cost, never as pure client CPU. **Records on the FAILURE path too**, not just success, so a storm's ~10 s failed probes land in the percentiles instead of being survivorship-filtered out of them (same exception-inclusive behaviour as `swath.api.latency`). `total` is recorded for every attempt, success or failure; `response_parse` is success-only by construction (a fetch that never returned a response has nothing to parse, and never fabricates a sample); `connect_acquire`/`ttfb`/`sdk_unmarshal` are recorded from whatever the SDK captured before the fault and are commonly unavailable on a fast-failing attempt (a hung read that never got a service response has no time-to-first-byte to report). `sdk_unmarshal` is deliberately **not** success-only, unlike `response_parse`: it is derived from the SDK's stamps, not swath's own timing, and the SDK still handles (and stamps) an ERROR response — so on a modeled service fault (a `SlowDown`, an `AccessDenied`) the sample recorded is a tiny error-document parse rather than a page's. Read the `sdk_unmarshal` distribution alongside its `count` and the run's error/throttle counters before comparing its p50 with a clean run's. **Caveats:** (1) `structure_probe` is NOT thief-exclusive — the seed step's own `delimiter=/` probes at run start classify identically, because the classifier sees only the request shape, never who issued it; (2) `pivot_probe` assumes the run's configured page size is `> 1` — a (never-recommended) `--max-keys=1` run misclassifies every ordinary worker page fetch as `pivot_probe`, since the classifier only checks `max_keys<=1` on the request |
| `swath.entries.emitted` | counter | — | object entries emitted downstream |
| `swath.open_frontier.keys_emitted` | counter | — | keys DURABLY committed AND kept post-filter while their node's upper bound was `null` — the serial-tail attribution gauge for the one range the owner-split governor can never self-split off (`OWNER_SPLIT.open_frontier`, §5). Counted at the same contract `swath.entries.emitted` is (past the page's durable commit, on the post-`FilterChain` kept count), so it can never read as a larger share than 1 of it. Absolute; divide by `swath.entries.emitted` for the share of the run's mass that drained from the open frontier — a large share paired with a slow tail means the tail sat here, un-carvable by the owner, rather than gate-blocked |
...
| `swath.parquet.write.latency` | timer | — | one stretch of WRITER-LANE work: the encode+write of a batch's rows into the open part, plus whatever finalize that stretch triggered (footer fsync, part MD5, manifest rewrite) — measured on the lane's own thread, between two waits on its queue. The pool's CPU is invisible to every other span (`swath.emit.latency` sees the pool DISPATCH only for Parquet), so this is what makes a Parquet run's client cost measurable rather than a lower bound: summed over a run it accounts for the pool's CPU. **Not page-scoped**: one observation per batch a lane wrote, plus one per idle-cadence rotation and one per lane's drain-time finalize/discard, so its count is `>=` the page count on a clean run (an aborted/failed run drains its queued batches without writing them, and those record nothing). A **client-service-cost span** (§3 `client_cost[]`, as `parquet_write`): publishes p50/p90/p99. **Overlaps in wall-clock, twice over** — the lanes run concurrently with fetch/emit (it is off the page's critical path, never a serial per-page cost), and it strictly CONTAINS the `swath.parquet.finalize.latency` sample of any rotation that fired inside the stretch |
...
| `swath.emit.latency` | timer | — | the consumer stage's per-page SINK-WRITE span: how long one page took to go through the installed output (format + write for the text sinks, pool dispatch for Parquet, lane admission for `--sort`), including that stage's own row tally. For Parquet this is the dispatch alone (plus any wait for lane-queue capacity, since `submit` blocks on a full lane queue) — the lanes' own encode/write is `swath.parquet.write.latency`, off this span. Exactly one consumer stage runs per run, so this is a single untagged series — which output produced it is already in `config.format`. Recorded only when the page went out intact (a broken pipe truncates it mid-write and is not a representative sample). A **client-service-cost span** (§3 `client_cost[]`): publishes p50/p90/p99 |
...
provider's pricing is therefore unknown. Read a missing key as a fact about *that run*, never as a
version mismatch.
...
```json
{
  ...
  "engine_flags": { "owner_split": true, "density_ewma": true, "radix_bands": true,
    ...
    "mass_aware_seed": true, "readahead": false, "rate_anchored_sensing": true,
    "tail_floor": "reach_floored", "max_duration_ms": null },
  ...
  "probe_latency": [
    { "call_class": "worker_page", "phase": "sdk_unmarshal", "count": 112, "p50_ms": 6.8, "p90_ms": 9.1, "p99_ms": 18.4, "max_ms": 44.0 },
  ...
  "client_cost": [
    ...
    { "span": "parquet_write", "count": 1123, "p50_ms": 1.9, "p90_ms": 3.6, "p99_ms": 12.4, "max_ms": 60.1 }
  ],
...
`client_cost[]`, `demand_gate`, and the `shape` feature-vector (all shown in the example above). Their field-by-field
